...
def test_conversation_agent_flow():
    ...
    # Query about SQL injection routing when SEC-002 is present
    mock_state["security_analysis"]["vulnerabilities"] = [{"id": "SEC-002", "line": 5, "title": "SQL Injection", "severity": "Critical", "classification": "Injection", "description": "SQL Injection", "snippet": "..."}]
    mock_state["chat_message"] = "Tell me about SQL injection"
    result_sqli = workflow.invoke(mock_state)
    assert "sql query string concatenation was detected" in result_sqli.get("chat_response").lower()

    # Query about Eval/Command injection routing when SEC-007 is present
    mock_state["security_analysis"]["vulnerabilities"] = [{"id": "SEC-007", "line": 8, "title": "Command Injection", "severity": "Critical", "classification": "Injection", "description": "Command Injection", "snippet": "..."}]
    mock_state["chat_message"] = "Tell me about eval exec or command injection"
    result_eval = workflow.invoke(mock_state)
    assert "eval/exec execution is flagged as critical" in result_eval.get("chat_response").lower()
